...
module ApplicationHelper
  ...
  def put_role_select(selected)
   roleids = Hash.new
   Configuration.roles.each do |role|
     roleids[role] = role
   end
  ...
  def display_title(event, search = nil)
    content_tag(:div,:class => "evtit#{event.tagged_with_title? ? ' ital' : ''}") do
      text = event.title ? highlight(sanitize(event.title), search, Configuration.found_text_replacement_string) : ''
  ...
  def display_vid_info(event)
    text = "<div class='evts'>"
      if event.video_id
        ...
        text << Configuration.no_video_string
      end
    text << '</div>'
  end
  ...
  def more_string(event)
    length = event.description.length - Configuration.truncate_length
  ...
  def display_description(event,search = nil)
    if (event.description)
      ...
      tt = case @truncate
        when :more then my_tags(truncate(event.description,:length => Configuration.truncate_length, :omission => more_string(event)))
        when :less then event.description.length > Configuration.truncate_length ? my_tags(event.description) + less_string(event) : my_tags(event.description)
        else  my_tags(event.description)
        end
      if search
        tt = highlight(tt,search,Configuration.found_text_replacement_string)
      end
      text << tt
      text << '</div>'
    else
  ...
  def display_subscenes(event,search = nil)
    ...
    event.sub_scenes.each do |ss|
      ...
      ss.description ||= ''
      if search
        ss.title = highlight(ss.title,search,Configuration.found_text_replacement_string)
        ss.description = highlight(ss.description,search,Configuration.found_text_replacement_string)
      end
      text << "#{ss.title}"
      text << '</span><br />&nbsp;&nbsp;'
      text << ss.description
